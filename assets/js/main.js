...
$(function() {
	...
	$.get('/taquillas/taquilla/getEdificios', function(data){
		...
		$('#campus').click(function(){
			...
			$('#tipoTaquilla').html('');
		/*	if($('#campus').val() == 2){
				$('#tipoTaquilla').html('<label>Simple 4€</label> 
					<input type="radio" name="simple" value="simple"><br> 
					<label>Doble 6€</label>
  					<input type="radio" name="doble" value="doble"><br>');
			}
			else{
				$('#tipoTaquilla').html('<label>Simple 6€</label> 
					<input type="radio" name="simpleCCSSJJ" value="simpleCCSSJJ"><br>');
			}*/
...
function generarEdificios(edificios) {
	...
	var resultado = "<option name='vacio' value='0'></option>\n";
